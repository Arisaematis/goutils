	...
	for _, t := range tasks {
		...
		go func(task FilePart) {
			...
			if err = d.downloadPart(task); err != nil {
				klog.Error(fmt.Sprintf("文件下载失败 %v %v", err, task))
			}
		}(t)
	}
	wg.Wait()
...
func (d *Downloader) GetFileSize() (uint64, error) {
	...
	klog.Info(fmt.Sprintf("要下载的文件大小为 %v byte\n", fileSize))
	return uint64(fileSize), err
}

func (d *Downloader) downloadPart(p FilePart) error {
	client := http.Client{}
	klog.Info(fmt.Sprintf("开始[%d]下载from:%d to:%d\n", p.Index, p.Start, p.End))
	req, err := http.NewRequestWithContext(context.Background(), "GET", d.url, nil)
	if err != nil {
		return err
	}
	...
	if err != nil {
		klog.Error("请求响应失败", err)
		return err
	}

	if resp.StatusCode > 299 {
		return fmt.Errorf(fmt.Sprintf("服务器状态码: %v", resp.StatusCode))
	}
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			klog.Error(fmt.Sprintf("关闭resp失败, %v", err))
		}
	}(resp.Body)
...
func (d *Downloader) merge() error {
	...
	for i := 0; i < d.taskNum; i++ {
		...
		if err != nil {
			klog.Error(fmt.Sprintf("合并文件时失败, %v\n", err))
			return err
		}
