...
func Values(v interface{}, uTag string) (url.Values, error) {
	...
	err := reflectValue(values, val, "", uTag)
	return values, err
}
...
// uTag user tag
func reflectValue(values url.Values, val reflect.Value, scope, uTag string) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if sv.Kind() == reflect.Struct {
			if err := reflectValue(values, sv, name, uTag); err != nil {
				return err
			}
			continue
		}
	...
	for _, f := range embedded {
		if err := reflectValue(values, f, scope, uTag); err != nil {
			return err
		}
	}
